...
pub fn enable_autostart(app: &mut tauri::App) {
    ...
    app.handle().plugin(tauri_plugin_autostart::init(MacosLauncher::AppleScript, None)).unwrap();

    let autostart_manager = app.autolaunch();

    match (autostart_manager.is_enabled(), current_autostart(app.app_handle())) {
        (Ok(false), Ok(true)) =>
...
pub fn change_autostart<R: Runtime>(app: tauri::AppHandle<R>, open: bool) -> Result<(), String> {
    ...
    let change = |open: bool| -> Result<(), String> {
       let open_str =  if open {
            autostart_manager.enable().map_err(|_| "enable autostart failed".to_owned())?;
           "true"
        } else {
            autostart_manager.disable().map_err(|_| "disable autostart failed".to_owned())?;
           "false"
        };
